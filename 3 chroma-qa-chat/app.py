...
def process_pdfs(pdf_storage_path: str):
    ...
    for pdf_path in pdf_directory.glob("*.pdf"):
        # Create a PyMuPDFLoader instance for the current PDF path
        loader = PyMuPDFLoader(str(pdf_path))
        # Use the loader to load the PDF content
        documents = loader.load()
         # Split the loaded PDF documents into smaller chunks using the text_splitter
        # and append the chunks to the docs list
...
# model_name specifies the version of the model to use, here it's "gpt-4".
# streaming=True enables a streaming interface for real-time processing of input and output.
# api_key is used for authentication with the OpenAI API.
